...
const completeUpload = async (assetId: number, uploadId?: string, parts?: Array<{PartNumber: number, ETag: string}>): Promise<void> => {
    ...
    // For multipart uploads, include upload_id and parts
    if (uploadId && parts) {
...
const uploadToAWS = async (url: string, data: Blob | File, originalFile?: File): Promise<string | null> => {
    // For AWS S3, we should NOT set Content-Type for multipart uploads
    // Let AWS handle it automatically based on the file extension
    const headers: Record<string, string> = {}
    
    // Only set content-type for single file uploads, not for chunks
    ...
    console.log(`Uploading to AWS:`, {
        ...
        size: data.size,
        type: data instanceof File ? data.type : 'blob',
        headers
    })
    ...
    // Return ETag for multipart uploads (remove quotes if present)
    const etag = response.headers.get('ETag')
    return etag ? etag.replace(/"/g, '') : null
}

const createChunkWithType = (file: File, start: number, end: number): Blob => {
    // Create a plain blob without content-type for multipart uploads
    // AWS S3 multipart uploads don't need content-type on individual parts
    const chunk = file.slice(start, end)
    return chunk // Return the raw blob without additional type specification
}

// Helper function to upload chunks with concurrency control
        ...
        mutationFn: async (file: File) => {
            ...
            try {
                ...
                if (isLargeFile) {
                    ...
                    const { data: { presigned_url, s3_key, asset_id } } = initResponse
